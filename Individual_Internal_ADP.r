...
final_clean <- output_clean_join %>% 
  ...
  mutate(DETLOC = case_when(City == "Binghamton" ~ "BROMMNY",
                            ...
                            Name == "Folkston D Ray Ice Processing Ctr" ~ "FIPCDGA",
...
final_group <- final_clean %>% 
  select(c(Name, City, State, DETLOC, `Pull Date`, Cumulative_beds, adp, `Level A`,
           ...
           `Male Non-Crim`, Cumulative_MaleNonCrim, `Female Crim`, Cumulative_FemaleCrim,
           `Female Non-Crim`, Cumulative_FemaleNonCrim, fiscal_year_day)) |>
...
grouped_selected <- final_group %>%
  select(Name, City, State, DETLOC, `Pull Date`,fiscal_year_day, adp, 
         ...
         starts_with("Cumulative"), ends_with("back_interval_adp")) %>% #Selecting only those columns that we required.
...
####### adding the Owner, Operator and Institutional type ####
operator_info <- openxlsx::read.xlsx('./ICE_Facility_Owner&Operator.xlsx') %>%
  select(DETLOC, Owner, Operator, Institutional.Type) #only pulling the 3 new columns so DETLOC is the sole join key and no Name/City/Address/State duplicates get created

final_table <- final_table %>%
  left_join(operator_info, by = "DETLOC")

filename <- paste0("IntervalAdpForIndividualColumn_",date,"_v1.xlsx") #saving the recent data's all backinterval into and excel file
...
data_before_the_latest_release <- grouped_selected %>%
  filter(`Pull Date` != latest_date)
# data_before_the_latest_release <- grouped_selected %>%
#   filter(!(`Pull Date` %in% c("2026-02-05", "2026-01-22")))
